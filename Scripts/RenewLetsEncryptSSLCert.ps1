...
#
# Script retrieved from this github repo
...
$tmpdir = $env:TEMP + "/" + (New-TimeSpan -Start (Get-Date "01/01/1970") -End (Get-Date)).TotalSeconds;
New-Item -ItemType directory -Path $tmpdir

# Create a state object and save it to the harddrive
$state = New-ACMEState -Path $tmpdir
...
$state = Get-ACMEState -Path $tmpdir;
...
Set-AzureStorageBlobContent -File $fileName -Container $StorageContainerName -Context $ctx -Blob $blobName
...
$certKey = New-ACMECertificateKey -Path "$tmpdir\$domain.key.xml";
...
Export-ACMECertificate $state -Order $order -CertificateKey $certKey -Path "$tmpdir\$domain.pfx" -Password $password;

# Delete blob to check DNS
Remove-AzureStorageBlob -Container $StorageContainerName -Context $ctx -Blob $blobName
...
Set-AzureRmApplicationGatewaySSLCertificate -Name $AGOldCertName -ApplicationGateway $appgw -CertificateFile "$tmpdir\$domain.pfx" -Password $password
